...
void PManager::ProcessServerPacket(unsigned char* recv)
{
	RecvBuf = recv;;
	unsigned char packet_type = recv[1];
...
{
	ss_packet_server_login_ok* packet = reinterpret_cast<ss_packet_server_login_ok*>(RecvBuf);
	server._sid = packet->server_id;
};

void PManager::restart()
{
	TimerEvent(Gm_id, Gm_id, GAME_OVER, 30000ms);
	cout << "RECYCLE " << endl;
